 ...
 * @since 22/12/2020
 * @version 1.0.3
 * @abstract SCRIPT QUE CONTEM FUNCOES GENERICAS PARA SEREM UTILIZADAS 
 *           NO DESENVOLVIMENTO DE JS DE MODELOS DO CLIENTE
 */
...
/**
 * SIMULA O EVENTO DE CLIQUE NO BOTAO QUE SALVA O FORMULARIO
 * 
 * @returns {boolean}
 */
const eSaveForm = () => {
    return new Promise((resolve, reject) => {
        let saveBtn = document.querySelectorAll('.saveActivity');
        try {
            saveBtn[0].click();
            resolve(true);
        } catch {
...
/**
 * SIMULA O EVENTO DE CLIQUE NO BOTAO QUE ABRE O FLUXO VIVO
 * 
 * @returns {boolean}
 */
const eOpenTimeLine = () => {
    return new Promise((resolve, reject) => {
        let timeLineBtn = document.querySelectorAll('.openTimeLine');
        try {
            timeLineBtn[0].click();
            resolve(true);
        } catch {
...
/**
 * SALVA O FORM E RECARREGA-O
 */
async function saveFormNReload() {
    let formSaved = await eSaveForm()
                                .then(getIfFormWasSaved => {
                                    return getIfFormWasSaved;
                                });
    console.info("Form foi salvo? " + formSaved);
    if (formSaved) 
        setTimeout(() => {
            eReloadFrame(formSaved);
        }, 1200);
    else 
        throw new Error("Não foi possível gravar os dados do processo.");
}

/**
 * SETA O EVENTO DE CLIQUE NO "BOTAO SALVAR" PERSONALIZADO PARA SEU FORM
 * 
 * @param {Integer} index - VARIA CONFORME A QTDE DE "BOTOES SALVAR" QUE VC CRIA NO SEU FORM
 */
const eSaveFormOnClick = (index) => {
    let saveBtn = document.querySelectorAll('.saveActivity');
    saveBtn[index].addEventListener("click", () => {
        saveBtn[0].click();
    });
}

/**
 * CRIA UM "BOTAO SALVAR" NO SEU FORM
 * 
 * @param {String} objectBeforeId - ID DO OBJETO ANTERIOR A INSERCAO DO BOTAO
 */
function loadSaveFormButton(objectBeforeId, index) {
    let divObjBefore = document.getElementById(objectBeforeId);
    let divSaveFormBtn = document.createElement("div");
    divSaveFormBtn.classList.add("activity-toolbox__actions");
    divSaveFormBtn.classList.add("activity-toolbox__actions_inner_form");

    let saveActivityBtn = document.createElement("button");
    saveActivityBtn.classList.add("button");
    saveActivityBtn.classList.add("btn");
    saveActivityBtn.classList.add("waves-effect");
    saveActivityBtn.classList.add("waves-light");
    saveActivityBtn.classList.add("button--icon");
    saveActivityBtn.classList.add("saveActivity");
    saveActivityBtn.classList.add("tooltipped");
    ...
    divInfo.classList.add("action-grid-info");
    let pInfo = document.createElement("p");
    pInfo.innerHTML = "Ao finalizar as edições na grid acima, salve as modificações no botão abaixo:";

    saveActivityBtn.appendChild(i);
    divSaveFormBtn.appendChild(saveActivityBtn);
    divInfo.appendChild(pInfo);
    divObjBefore.after(divInfo);
    divInfo.after(divSaveFormBtn);

    $(".action-grid-info").css("color", "#FC0000");
    $(".action-grid-info").css("font-size", "12px");
    $(".action-grid-info").css("margin", "0 auto 1rem");
    $(".action-grid-info").css("position", "relative");
    $(".action-grid-info").css("left", "44%");
    $(".action-grid-info").css("height", "20px");
    $(".action-grid-info").css("width", "90%");

    $(".activity-toolbox__actions_inner_form").css("position", "relative");
    $(".activity-toolbox__actions_inner_form").css("left", "89%");
    $(".activity-toolbox__actions_inner_form").css("top", "1.8rem");
    $(".activity-toolbox__actions_inner_form").css("margin", "0 auto 3rem");

    eSaveFormOnClick(index);
 ...
 * @return {Object Array}
 */
const getCamposTipo = function (tipoCampo) {
...
function setFieldsDisabled(fields, isDisabled) {
    for (let field of fields) getForm(field).disabled(isDisabled);
    Form.apply();
}
...
function setFieldsRequired(fields, action, isRequired) {
    for (let field of fields) getForm(field).setRequired(action, isRequired);
    Form.apply();
}
...
function setFieldsReadOnly(fields, isReadOnly) {
    for (let field of fields) getForm(field).readOnly(isReadOnly);
    Form.apply();
}
...
function clearListOptions(listFields) {
    for (let listField of listFields) getForm(listField).removeOptions([]).apply();
    Form.apply();
}
 ...
 */
function alterarLarguraCampoNormal(nomeCampo, novoTamanho) {
    try {
        getForm(nomeCampo).className("col " + novoTamanho).apply();
    } catch (error) {
        // console.log("alterarLarguraCampoNormal.error:\r\n" + error);
    }
}

/**
 * ALTERA TEXTO DO LABEL DE UM CAMPO
 * UTIL QDO O TEXTO ULTRAPASSA O TAMANHO PERMITIDO PELO PRODUTO
 * 
 * @param {String} campo - CAMPO QUE TERA O VALOR DO LABEL ALTERADO
 * @param {String} txt - TEXTO QUE SERA INSERIDO
 */
function alteraLabelCampo(campo, txt) {
    let campoObj = document.querySelector("#input__" + campo + " .label > span");
    campoObj.innerHTML = txt;
}
 ...
 * @requires getForm
 */
function alteraLabelCampoApi(campo, txt) {
 ...
 * @param {Array} fields
 */
function openCustomModal(headerTitle, msg, clearFields, fields) {
 ...
 */
function displayCustomModal(title, description) {
    Form.addCustomModal({
        title,
        description,
...
function cleanFieldsValue(fields) {
    for (let field of fields) getForm(field).value('')
    Form.apply();
}
 ...
 */
function ocultaCamposVazios(idGrupos) {
    try {
        ...
        console.log("ocultaCamposVazios.error:\r\n" + error);
    }
}

// --------------------------------------------------------------------------------
// CALCULOS ENTRE CAMPOS NUMERICOS OU MOEDA
// --------------------------------------------------------------------------------
function variacaoPercentualDesconto(valorMaior, valorMenor) {
    return (((valorMaior - valorMenor) * 100) / valorMaior).toFixed(2);
}

function variacaoPercentualAcrescimo(valorMaior, valorMenor) {
    return (((valorMaior - valorMenor) * 100) / valorMenor).toFixed(2);
}
 ...
 * @returns {int} 
 */
function getDataRowsCount(idGrid) {
 ...
 */
function setColumnsVisibility(grid, columns, isVisible) {
 ...
 */
function somaValorGrid(grid, nomeCampoValor, nomeCampoTotal) {
 ...
 */
function maiorValorGrid(grid, nomeCampoValor, nomeCampoMaximo) {
    Form.apply().then(function () {
 ...
 */
function menorValorGrid(grid, nomeCampoValor, nomeCampoMinimo) {
    Form.apply().then(function () {
 ...
 * @param {Object} grid - NOME DA GRID
 * @param {Integer} quantidade - QUANTIDADE DE LINHAS MINIMAS
 * @param {String} mensagemErro - MENSAGEM DE ERRO
 */
function validaPreenchimentoMinimoItensGRID(grid, quantidade, mensagemErro) {
    Form.apply().then(function () {
        var tamanhoGrid = getDataRowsCount(grid);
        if (tamanhoGrid < quantidade) {
            Form.subscribe('SUBMIT', function (formId, actionId, reject) {
                Form.addCustomModal({ title: "AVISO", description: mensagemErro, buttons: [] });
                reject();
            });
        }
    });
 ...
 */
function validaPreenchimentoMinimoItensNGrids(idGrids) {
...
/**
 * VALIDA SE A GRID TEM INFORMACAO COM AO MENOS UM ITEM PREENCHIDO COM O VALOR X
 * @param {Object} grid - OBJETO DOM DA GRID
 * @param {String } nomeCampo - NOME DO CAMPO QUE SERA VALIDADO SE POSSUI  O VALOR
 * @param {*} valor - VALOR A SER VALIDADO SE EXISTE NA GRID
 * @param {String} mensagemErro - MENSAGEM DE ERRO
 */
function validarGridPossuiValorDefinido(grid, nomeCampo, valor, mensagemErro) {
    Form.apply().then(function () {
        var expensiveDataRows = grid.dataRows(function (dataRow) {
            return dataRow[nomeCampo] == valor;
        });
        if (expensiveDataRows != undefined && expensiveDataRows < 1) {
            Form.subscribe('SUBMIT', function (formId, actionId, reject) {
                Form.addCustomModal({ title: "AVISO", description: mensagemErro, buttons: [] });
                reject();
            });
 ...
 */
function ocultarExibirGrid(grid, isVisible) {
 ...
 * @param {boolean} ocultar 
 */
function ocultarExibirCamposGrid(grid, campos, isVisible) {
 ...
 */
function ocultaLinhaGRID(nomeGrid, nomeCampo, naoOcultarValor) {
 ...
 */
function gridAlteraLabelBtn(idGrid, labelAdicao, labelEdicao) {
    gridAlteraLabelBtnChild(idGrid, "CREATE", labelAdicao, labelEdicao);
    ...
    Form.grids(idGrid).subscribe("GRID_EDIT", function () {
        gridAlteraLabelBtnChild(idGrid, "UPDATE", labelAdicao, labelEdicao);
    });
}
function gridAlteraLabelBtnChild(idGrid, acao, labelAdicao, labelEdicao) {
    setTimeout(function () {
...
function gridOcultaBotaoCreate(idGrid, acao, isHidden) {
    setTimeout(() => {
 ...
 */
function gridDesabilitaEdicoesExclusoes(
    idGrid,
    desabilitarEdicoes,
    ...
    try {
        ...
        setTimeout(function () {
            ...
            // ----------------------------------------------------------------------------------------
            gridDesabilitaEdicoesExclusoesChild(
                idGrid,
                desabilitarEdicoes,
                desabilitarExclusoes,
                indexList
        ...
        console.log("gridDesabilitaEdicoesExclusoes.error:\r\n" + error);
    }
}
function gridDesabilitaEdicoesExclusoesChild(
    idGrid,
    desabilitarEdicoes,
    desabilitarExclusoes,
    indexList
    ...
    try {
        ...
        gridDesabilitaEdicoesExclusoesChild2(gridRows, indexList, display);
        // desabilitarExclusoes;
        gridRows = document.querySelectorAll("#input__" + idGrid + " #destroy");
        display = desabilitarExclusoes ? "none" : "inline-block";
        gridDesabilitaEdicoesExclusoesChild2(gridRows, indexList, display);
    } catch (error) {
        console.log("gridDesabilitaEdicoesExclusoesChild.error:\r\n" + error);
    }
}
function gridDesabilitaEdicoesExclusoesChild2(gridRows, indexList, display) {
    try {
        for (let index = 0; index < gridRows.length; index++) {
            let check = true;
        ...
        console.log("gridDesabilitaEdicoesExclusoesChild2.error:\r\n" + error);
    }
}
 ...
 */
function gridInibeInsercaoNovaLinha(grid, maxItens, mensagem) {
 ...
 * @param {Object} grid - OBJETO DOM DA GRID
 */
function gridRemoveUltimaLinhaInserida(grid) {
 ...
 */
function gridGetValorColunaUltimaLinhaAdicionada(grid, idCampo) {
    let gridRows    = grid.dataRows();
 ...
 */
function gridGetValoresJaInseridos(grid, idCampo) {
 ...
 * @returns {boolean}
 */
function gridValidarValorJaInserido(valorVerificar, valoresJaInseridos) {
 ...
 * @returns {boolean}
 */
const gridVerificaDuplicidade = (grid, idCampo, valorVerificar) => {
    return new Promise( (resolve, reject) => {
 ...
 * @returns {Boolean}
 */
function validarDataMaiorQueCorrente(campo) {
    Form.apply().then(function () {
 ...
 */
function formataDataTipoI(data) {
...
// --------------------------------------------------------------------------------
// ALTERAR ESTILOS DEFAULT DO FORM APP
// --------------------------------------------------------------------------------
/**
 * 
 * @param {String} nomeCampo - NOME DO CAMPO DO TIPO CAIXA DE TEXTO
 * @description A OPCAO "Mostrar Historico" DEVE ESTAR HABILITADA NO CAMPO INFORMADO
 */
function estilizarHistorico(nomeCampo) {
    if (getForm(nomeCampo).readOnly() == true) {
        $('#' + nomeCampo).hide()
        $('#input__' + nomeCampo + ' > div > label > span').css({
            'color': '#000000', 'font-weight': 'bold' // Nome do campo em negrito
        })
    }
    if (getForm(nomeCampo).readOnly() == false) {
        $('#history__field-' + nomeCampo).css({
            'margin-top': '-5px' // para quando o campo estiver disponível, diminuir espaçamento
        })
    }
    $('#history__field-' + nomeCampo + ' > div').css({
        'padding': 'inherit', 'margin-bottom': '15px' // divisor da box - history container
    })
    for (let i = 0; i <= 100; i++) {
        $('#history__field-' + nomeCampo + ' > div > div:nth-child(' + i + ')').css({
            'margin-bottom': '3px' // divisor entre os históricos do mesmo campo
        })
    }
    $('#history__field-' + nomeCampo + ' > div > div > div.history__item-header > p').css({
        'color': '#2273DA', 'font-size': '12px' // azul || etapa - responsável - data
    })
    $('#history__field-' + nomeCampo + ' > div > div > div.history__description').css({
        'font-size': '12px', 'color': '#9E9E9E' // texto histórixo
    })
    $('#history__field-' + nomeCampo + ' > a > span').css({
        'font-size': '12px' // botão esconder/mostrar histórico
    })
    $('#history__field-' + nomeCampo + ' .history__container').addClass('hide')
    $('#history__field-' + nomeCampo + ' > a > i').html('keyboard_arrow_down')
    $('#history__field-' + nomeCampo + ' > a > span').html('Mostrar histórico')
}
 ...
 */
function estilizaRadiosAlinhamentoHorizontal(nomeCampo) {
